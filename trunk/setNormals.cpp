...
char inFileNameNormals[32]="input",outFileNameNormals[32]="output";
...
int parseCommandNormals(int argc,char **argv) {
    ...
    for(int i=1;i<argc;) {
        ...
        if(argv[i][0]!='-')
            return 0;
        switch(argv[i][1]) {
        case 'o':strcpy(outFileNameNormals,argv[i+1]);
            i=i+2;
            break;
        case 'i':strcpy(inFileNameNormals,argv[i+1]);
            i=i+2;
            break;
...
int mainNormals(int argc,char** argv) {

    powercrust crust;
    ifstream inFile;
    ofstream outFile;
    if(!parseCommandNormals(argc,argv)) {
        cout<<"Error in arguments..\n";
        exit(0);
    }

    inFile.open(inFileNameNormals);
    outFile.open(outFileNameNormals);
